...
(defn palabra-reservada? [x]
  (let [palabras-reservadas '("CONST", "VAR", "PROCEDURE", "CALL", "BEGIN", "END", "IF", "THEN", "WHILE", "DO", "ODD", "READLN", "WRITELN", "WRITE")]
    (if (some #{(str x)} palabras-reservadas) true false)))
...
(defn identificador? [x]
  (if (or (nil? (re-matches #"[A-Z][A-Z1-9]*" (str x))) (palabra-reservada? x)) false true))
...
(defn inicializar-contexto-local [amb]
  (if (= (estado amb) :sin-errores)
    (assoc amb 4 (assoc (contexto amb) 0 (conj (first (contexto amb)) (count (nth (contexto amb) 1)))))
    amb))
...
(defn expresion [amb]
(if (= (estado amb) :sin-errores)
  (if (or (= '+ (simb-actual amb)) (= '- (simb-actual amb)))
    (generar-signo
      (-> amb
        (escanear)
        (termino)
...
(defn buscar-coincidencias [amb]
  (filter (fn [x] (= (first x) (last (simb-ya-parseados amb)))) (nth (contexto amb) 1)))
